...
export function LegalDeadlineDialog({ process, deadline, open, onOpenChange, onSuccess }: LegalDeadlineDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl bg-card border-border h-[90vh] flex flex-col p-0 overflow-hidden shadow-2xl">
        ...
        <ScrollArea className="flex-1">
          <Form {...form}>
            <form id="deadline-form" onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-6">
        ...
        <DialogFooter className="p-6 border-t border-white/5 bg-white/5 shrink-0 gap-2">
          <DialogClose asChild>
            <Button variant="ghost" type="button" className="text-slate-400 hover:text-white font-bold uppercase text-[10px]">Cancelar</Button>
          </DialogClose>
          <Button 
            type="submit" 
            form="deadline-form"
            disabled={isSaving} 
            className="bg-primary text-primary-foreground hover:bg-primary/90 font-black uppercase tracking-widest text-[11px] px-8 h-11 shadow-xl shadow-primary/20"
          >
