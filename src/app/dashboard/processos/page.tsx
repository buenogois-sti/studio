...
const processSchema = z.object({
  ...
  opposingParties: z.array(z.object({ value: z.string().min(1, { message: 'O nome não pode ser vazio.' }) })).optional(),
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {value ? `${value.firstName} ${value.lastName}` : 'Selecione um cliente...'}
...
function ProcessForm({ onSave, process }: { onSave: () => void; process?: Process | null }) {
  ...
  async function onSubmit(values: z.infer<typeof processSchema>) {
    ...
    try {
      ...
      if (process?.id) {
        ...
        await updateDoc(processRef, { ...processData, updatedAt: serverTimestamp() });
        toast({ title: 'Processo atualizado!', description: `O processo "${values.name}" foi salvo.` });
      } else {
        ...
        toast({ title: 'Processo cadastrado!', description: `O processo "${values.name}" foi adicionado com sucesso.` });
      }
      onSave();
    } catch (error: any) {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <section>
                        ...
                        <FormField
                            ...
                            <FormItem>
                                ...
                                <FormControl>
                                <Input placeholder="Ex: Reclamação Trabalhista vs Empresa X" {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                            )}
                        ...
                        <FormField
                            ...
                            <FormItem>
                                ...
                                <FormControl>
                                <Input placeholder="Ex: 80ª Vara do Trabalho de São Paulo" {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                            )}
                        ...
                        <FormField
                            ...
                            <FormItem>
                                ...
                                <FormControl>
                                <Input type="number" placeholder="R$ 10.000,00" {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                            )}
                    ...
                    <section>
                         ...
                         <FormField
                            ...
                            <FormItem className='mt-4'>
                                <FormControl>
                                <Textarea placeholder="Detalhes, andamentos, links importantes..." className="min-h-[150px]" {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                            )}
            ...
            <SheetFooter className="pt-4 pr-4">
                ...
                <Button type="submit" disabled={isSaving}>
                    ...
                    {isSaving ? 'Salvando...' : (process ? 'Salvar Alterações' : 'Salvar Processo')}
                </Button>
            </SheetFooter>
        </form>
      </Form>
      ...
      <Sheet open={isClientSheetOpen} onOpenChange={setIsClientSheetOpen}>
        <SheetContent className="sm:max-w-2xl w-full">
            <SheetHeader>
                ...
                <SheetDescription>
                Preencha os dados para cadastrar um novo cliente. Ele será selecionado automaticamente após o cadastro.
                </SheetDescription>
            </SheetHeader>
                ...
                {isLoading ? (
                  ...
                  processesData.map((process) => (
                    <TableRow key={process.id}>
                      ...
                      <TableCell>{clientsMap.get(process.clientId) || 'Cliente não encontrado'}</TableCell>
                      <TableCell className="hidden md:table-cell">
                        {process.processNumber || 'Não distribuído'}
                      </TableCell>
                       <TableCell className="hidden md:table-cell">
                         <Badge variant={process.status === 'Ativo' ? 'secondary' : (process.status === 'Arquivado' ? 'outline' : 'default')}
                            className={cn({
                                'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300': process.status === 'Ativo',
                                'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300': process.status === 'Pendente',
                            })}>
                            {process.status}
                        </Badge>
                      </TableCell>
      ...
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <SheetContent className="sm:max-w-2xl w-full">
          <SheetHeader>
            ...
            <SheetDescription>
              {editingProcess ? 'Altere os dados do processo abaixo.' : 'Preencha os dados para cadastrar um novo processo.'}
            </SheetDescription>
          </SheetHeader>
...
// Separator for Command component
